...
namespace CovTestMgmt.Application.IntegrationTests
{
    [SetUpFixture]
    public class Testing
    {
        ...
        public void RunBeforeAnyTests()
        {
            ...
            services.AddSingleton(Mock.Of<IWebHostEnvironment>(
                ...
                w.ApplicationName == "CovTestMgmt.API"
            ));
        ...
        {
            return await RunAsUserAsync("mail@jas.bio");
        }

        public static async Task<Guid> RunAsUserAsync(string email)
        {
            using var scope = _scopeFactory.CreateScope();

            var command = new LoginOrCreateNewUserCommand { Email = email };
            ...
            {
                throw new Exception($"Unable to create {email} {e.Message}");
            }
        }
